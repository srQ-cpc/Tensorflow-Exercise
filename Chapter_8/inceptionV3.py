...
def build_model_fine_tuning(model, learning_rate=0.0001, momentum=0.9):
        ...
        model.compile(optimizer=SGD(learning_rate=learning_rate, momentum=momentum), loss='binary_crossentropy', metrics=['accuracy'])
        return model
...
def main():
    ...
    scores = model.evaluate(val_generator, steps=val_generator.n // batch_size)
